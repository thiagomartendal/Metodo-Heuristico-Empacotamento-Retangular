...
// O método totalItens obtém o número total de itens empacotados na folha
int Executar::totalItens(std::vector<std::pair<int, std::vector<Item>>> conjuntos) {
...
// O método resultados exibe na telo os dados da execução de uma instância
void Executar::resultados(long long int lucroTotal, int totalFilas, int totalItensAlocados, long long int areaOcupada, long double perdaCorte, double taxaOcupacao, int tempoTotal) {
...
// O método instancia realiza a execução de uma instância de dados na heurística
DadosExecucao Executar::instancia(std::string tipoLucro, std::string tipoMochila, std::string instancia, int numArquivo) {
