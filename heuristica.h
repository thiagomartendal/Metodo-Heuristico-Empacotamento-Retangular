...
private:
    ...
    std::tuple<long long int, int, std::vector<Item>> procedimentoMochila(
        int alturaLivre, std::string tipoMochila);
    void procurarEspacosLivres(int x, int y, int maiorAltura,
    ...
    void empacotarEspacosLivres(long long int &lucroAtual,
        long long int &areaOcupada, std::vector<Item> &itens,
        std::vector<Espaco> &espacosLivres, std::string tipoMochila);

public:
    Heuristica(int larguraFolha, int alturaFolha, std::vector<int> &itens,
        std::vector<int> &larguras, std::vector<int> &alturas, std::string tipoLucro);
    std::tuple<long long int, long long int,
    std::vector<std::pair<int, std::vector<Item>>>> formarConjuntos(
